...
def fetch_papers(
    search_query, start=0, total_results=10, results_per_iteration=10, wait_time=3
):
...
def clean_text(text):
    ...
    abstract_to_conclusion = re.search(
        r"ABSTRACT(.*?)CONCLUSION", text, flags=re.DOTALL | re.IGNORECASE
    )
